...
std::vector<cudaGraphNode_t> get_current_capture_dependencies(cudaStream_t s) {
	...
	cudaError_t err = cudaStreamGetCaptureInfo(s, &status, nullptr, nullptr, ((const cudaGraphNode_t**)&depNodes), nullptr, &numDeps);
...
void pollingKernel(TimelineSemaphore* gpu1_complete_flag, uint64_t value, cudaStream_t s) {
	...
	if (1 || !is_stream_being_captured(s)) {
		...
		cudaStreamGetCaptureInfo(s, &capture_status, nullptr, &capturing_graph, &deps, nullptr, &dep_count);
		...
		cudaStreamUpdateCaptureDependencies(s, &peer_kernel_node, nullptr, 1, 1);
	}
}
...
bool is_stream_being_captured(cudaStream_t stream) {
	cudaStreamCaptureStatus capture_status;
	cudaStreamGetCaptureInfo(stream, &capture_status, nullptr, nullptr, nullptr, nullptr, nullptr);
...
void verify_all_streams_joined(cudaStream_t main_stream) {
	...
	if (status == cudaStreamCaptureStatusActive) {
		...
		cudaGraph_t capturing_graph;
		cudaStreamGetCaptureInfo(main_stream, &status, NULL, &capturing_graph, NULL, NULL, NULL);
...
void printGraphDependencies(cudaGraph_t graph, const char* name) {
	...
	for (size_t i = 0; i < num_nodes; i++) {
		...
		size_t num_incoming;
		cudaGraphNodeGetDependencies(nodes[i], NULL, nullptr, &num_incoming);

		// Outgoing edges (dependents)
		size_t num_outgoing;

		cudaGraphNodeGetDependentNodes(nodes[i], NULL, nullptr, &num_outgoing);
		...
		if (num_incoming > 0) {
			...
			cudaGraphNodeGetDependencies(nodes[i], deps.data(), nullptr, &num_incoming);
		...
		if (num_outgoing > 0) {
			...
			cudaGraphNodeGetDependentNodes(nodes[i], dependents.data(), nullptr, &num_outgoing);
	...
	for (size_t i = 0; i < num_nodes; i++) {
		size_t num_deps;
		cudaGraphNodeGetDependencies(nodes[i], NULL, nullptr, &num_deps);
		if (num_deps == 0) {
			printf("[%zu] ", i);
			source_count++;
		}
	...
	for (size_t i = 0; i < num_nodes; i++) {
		...
		cudaGraphNodeGetDependentNodes(nodes[i], NULL, nullptr, &num_dependents);
		if (num_dependents == 0) {
			printf("[%zu] ", i);
			sink_count++;
		}
	...
	for (size_t i = 0; i < num_nodes; i++) {
		...
		cudaGraphNodeGetDependentNodes(nodes[i], NULL, nullptr, &num_deps);
...
bool pathExists(cudaGraphNode_t source, cudaGraphNode_t sink, const std::vector<cudaGraphNode_t>& all_nodes, std::set<uintptr_t>& visited) {
	...
	size_t num_dependents;
	cudaGraphNodeGetDependentNodes(source, NULL, nullptr, &num_dependents);

	if (num_dependents > 0) {
		std::vector<cudaGraphNode_t> dependents(num_dependents);
		cudaGraphNodeGetDependentNodes(source, dependents.data(), nullptr, &num_dependents);
...
void printGraphDependencies2(cudaGraph_t graph, const char* name) {
	...
	if (sink_nodes.size() > 1) {
		...
		for (size_t sink_idx : sink_nodes) {
			size_t num_incoming;
			cudaGraphNodeGetDependencies(nodes[sink_idx], NULL, nullptr, &num_incoming);
			CudaCheckErrorModNoSync;
			printf("Sink[%zu]: Has %zu incoming dependencies\n", sink_idx, num_incoming);

			// Get dependencies for this sink
			std::vector<cudaGraphNode_t> deps(num_incoming);
			cudaGraphNodeGetDependencies(nodes[sink_idx], deps.data(), nullptr, &num_incoming);
			CudaCheckErrorModNoSync;
			printf("  Depends on: ");
...
void transferKernel(float* src, float* dst, size_t elems, cudaStream_t s, size_t involved_sm) {
	...
	if (is_stream_being_captured(s)) {
		...
		size_t _dep_count;
		cudaStreamGetCaptureInfo(s, &_capture_status, nullptr, &_capturing_graph, &_deps, nullptr, &_dep_count);
		...
		cudaStreamUpdateCaptureDependencies(s, &copy_0to1, nullptr, 1, 1);
