	...
	for (int j = 0; j < 2; ++j) {
		stream[j]->wait(part[j]->s);
		{
			...
			for (int i = 0; i < size; i += batch) {
				...
				NTT_<false, algo, mode><<<dim3{ cc.N / (blockDimFirst.x * M * 2), num_limbs }, blockDimFirst, bytesFirst, stream[j]->ptr()>>>(
								...
								for (size_t j = 0; j < cc.GPUid.size(); ++j) {
									if (j != i) {
										if (j == id) {
											...
											if (0 && GRAPH_CAPTURE) {
												CudaCheckErrorModNoSync;
												pollingKernel(signal[d][j][i].second, signal[d][j][i].first + 2, stream.ptr());
												CudaCheckErrorModNoSync;
												// signal[d][j][i].first++;
												CudaCheckErrorModNoSync;
											} else {
...
void LimbPartition::moddownMGPU(LimbPartition& auxLimbs, bool ntt, bool free_special_limbs, const std::vector<uint64_t*>& bufferSpecial_) {
	...
	if (limb_size > 0) {
		...
		// stream.wait(auxLimbs.s);
		{
			...
			int shared_bytes = sizeof(uint64_t) * (SPECIALlimb.size()) * blockSize.x;
			ModDown2<ALGO_SHOUP>
			  <<<gridSize, blockSize, shared_bytes, stream.ptr()>>>(auxLimbs.limbptr.data, limb_size, auxLimbs.SPECIALlimbptr.data, PARTITION(id, 0), getGlobals());
			CudaCheckErrorModNoSync;
		}
	}
